...
class HorshoeVortex:
    ...
    def __init__(self, LE_point_1, TE_point_1, LE_point_2, TE_point_2, aerodynamic_center_location = 0.25):
        ...
        self.filaments.append(BoundFilament(x1 = bound_point_1, x2 = bound_point_2)) # bound vortex
        self.filaments.append(BoundFilament(x1 = TE_point_1, x2 = bound_point_1)) # trailing edge vortex
        self.filaments.append(BoundFilament(x1 = bound_point_2, x2 = TE_point_2)) # trailing edge vortex
...
class BoundFilament:
    ...
    def calculate_induced_velocity(self, point,gamma = 1.0):
        ...
        if np.linalg.norm(r1Xr0)/self.length > epsilon: # Perpendicular distance from XVP to vortex filament (r0)
            ...
            vel_ind = gamma/(4*np.pi)*r1Xr2/(np.linalg.norm(r1Xr2)**2)*np.dot(r0, r1/np.linalg.norm(r1)-r2/np.linalg.norm(r2))
        else:
            # The control point is placed on the edge of the radius core
            # proj stands for the vectors respect to the new controlpoint
            r1_proj = np.dot(r1, r0)*r0/(self.length**2)+epsilon*r1Xr0/np.linalg.norm(r1Xr0)    
            r2_proj = np.dot(r2, r0)*r0/(self.length**2)+epsilon*r2Xr0/np.linalg.norm(r2Xr0)
            r1Xr2_proj = np.cross(r1_proj, r2_proj)
            vel_ind_proj = gamma/(4*np.pi)*r1Xr2_proj/(np.linalg.norm(r1Xr2_proj)**2)*np.dot(r0, r1_proj/np.linalg.norm(r1_proj)-r2_proj/np.linalg.norm(r2_proj))
...
class SemiInfiniteFilament:
    ...
    def calculate_induced_velocity(self, point,gamma,Uinf):
        ...
        r_perp =  np.dot(r1, self.direction)*self.direction                       # Vector from XV1 to XVP perpendicular to the core radius 
        epsilon = np.sqrt(4*self.alpha0*self.nu*np.linalg.norm(r_perp)/Uinf)   # Cut-off radius 

        if np.linalg.norm(r1XVf)/np.linalg.norm(self.direction) > epsilon:   
            # determine scalar
            K=gamma/4/np.pi/np.linalg.norm(r1XVf)**2*(1+np.dot(r1, self.direction)/np.linalg.norm(r1))
            # determine the three velocity components
            vel_ind = K*r1XVf
        else:
            r1_proj = np.dot(r1, self.direction)*self.direction+epsilon*(r1/np.linalg.norm(r1)-self.direction)/np.linalg.norm(r1/np.linalg.norm(r1)-self.direction)
            r1XVf_proj = np.cross(r1_proj, self.direction)
            K=gamma/4/np.pi/np.linalg.norm(r1XVf_proj)**2*(1+np.dot(r1_proj, self.direction)/np.linalg.norm(r1_proj))
            # determine the three velocity components
