...
CL1 = np.zeros(len(aoas)); CL2 = np.zeros(len(aoas))
CD1 = np.zeros(len(aoas)); CD2 = np.zeros(len(aoas))
Gamma0 = np.zeros(N-1)
for i in range(len(aoas)):
    ...
    #Output forces
    F_rel,F_gl,Ltot,Dtot,CL1[i],CD1[i],CS= VSM.output_results(Fmag,aero_coeffs,ringvec,
                                                            Uinf,controlpoints,Atot)
    ...
    ringvec, controlpoints, rings,Uinf,Gamma0,data_airf,conv_crit,model)
    #Output forces
    F_rel,F_gl,Ltot,Dtot,CL2[i],CD2[i],CS = VSM.output_results(Fmag,aero_coeffs,ringvec,Uinf,controlpoints,Atot)
