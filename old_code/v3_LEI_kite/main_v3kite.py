...
Uinf = np.array([np.cos(aoa)*np.cos(ss),np.sin(ss),np.sin(aoa)])*Umag # Wind speed vector
...
for aoa in alphas:
    for ss in sideslips:
        ...
        #%OUTPUT Results
        F_rel,F_gl,Ltot,Dtot,CLi,CDi,CSi = VSM.output_results(Fmag,aero_coeffs,ringvec,Uinf,controlpoints,Atot)
        aoa_map.append(aoa)
        ss_map.append(ss)
        CL.append(CLi)
        CD.append(CDi)
