...
Gamma0 = 0
Gamma_i = VSM.get_circulation_distribution_elliptical_wing(
    Gamma0, span, N
)  # Ellyptical wing Gamma distribution
...
F_relv, CLv, CDv, CSv, Gammav, aero_coeffsv = VSM.run_VSM(
    coord, Uinf, Gamma_i, data_airf, conv_crit_damp, Atot, rho=1.225
)
print(CLv, CDv)
t2 = time.time()
Gamma_i = VSM.get_circulation_distribution_elliptical_wing(Gamma0, span, N)
F_rel, CL, CD, CS, Gamma, aero_coeffs = VSM.run_VSM(
    coord, Uinf, Gamma_i, data_airf, conv_crit_nodamp, Atot, rho=1.225
)
print(CL, CD)
t3 = time.time()
...
plt.plot(
    coord_L[:, 1] / max(coord_L[:, 1]), aero_coeffsv[:, 1], label="Artificial visc."
)
plt.xlabel(r"$y/2b$")
plt.ylabel(r"$C_l$")
plt.legend()
...
plt.figure(figsize=(6, 4))
plt.plot(
    coord_L[:, 1] / max(coord_L[:, 1]),
    aero_coeffs[:, 0] * 180 / np.pi,
    label="No visc.",
)
plt.plot(
    coord_L[:, 1] / max(coord_L[:, 1]),
    aero_coeffsv[:, 0] * 180 / np.pi,
    label="Artificial visc.",
)
plt.xlabel(r"$y/2b$")
plt.ylabel(r"$\alpha$")
# plt.ylim((0,25))
...
plt.plot(
    coord_L[:, 1] / max(coord_L[:, 1]), aero_coeffsv[:, 2], label="Artificial visc."
)
plt.xlabel(r"$y/s$")
